...
class PlacesApiService{
  ...
  Future<List<Item>> fetchHotels(double lat, double long) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        List<Item> hotels = parseItemResponse(response.body, true);
        return hotels;
      }
      else {
  ...
  Future<List<Item>> fetchAttractions(double lat, double long) async{
    /*if(lat > 7){
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Item> attractions = parseItemResponse(response.body, false);
  ...
  List<Item> parseItemResponse(String responseBody,bool isHotel){
  ...
  Future<List<Item>> fetchNextPage(bool isHotel) async {
    String nextUrl = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?";
    if(isHotel){
        if(hotelsPageToken == ""){
          throw NoNextPageException();
        }
        nextUrl += "pagetoken=$hotelsPageToken&key=$_apiKey";
    }
    else{
      if(attractionsPageToken == ""){
        throw NoNextPageException();
      }
      nextUrl += "pagetoken=$attractionsPageToken&key=$_apiKey";
    }
    print("hotelsPageToken: " + hotelsPageToken);
    print("FetchNextPage url : " + nextUrl);
    http.Response response = await http.get(Uri.parse(nextUrl));
    if(response.statusCode == 200){
      List<Item> hotels =  parseItemResponse(response.body, false);
      return hotels;
    }
    else{
      print("Error while loading inside placesApiService...");
  ...
  Future<List<PlaceSearch>> getAutocomplete(String search,String type) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print("getAutocompleteSerch called ...");
